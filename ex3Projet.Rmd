...
**Pour des raisons principalement esthétiques, tout le code utilisée n'est pas présenté ici. Pour voir l'entièreté du code, voir ce [Github](https://github.com/JosephBARBIERDARNAL/RDSMTD1ex3).**
...
> Tous les prochains graphiques seront présentés de telle sorte : les graphiques de gauche représentent les pays à l'année 1957 et les graphiques de droite à l'année 2002.
...
Etant donné l'existence de fortes disparités dans le PIB/habitant entre les différents continents, il serai peu pertinent de créer un graphique sur une même échelle. Pour le prochain graphique, nous décidons alors de transformer notre variable PIB/habtiant en z-score, de la manière suivante : 
...
On s'intéresse maintenant à la distribution du PIB/habitant de l'Asie en 1957 et 2002 afin d'illustrer le constat précédent.
...
```{r echo=FALSE, fig.width=10}
plot1 = ggplot(data_continent_1957, aes(x=gdp_m, y=lifexp_m, col=continent)) +
  geom_point(size=4) +
  ylim(35, 85) +
  ylab("Espérance de vie (en 1957)") +
  xlim(1000, 28000) +
  xlab("PIB/habitant (en 1957)") +
  geom_errorbar(aes(ymin=lifexp_m-lifexp_sd, ymax=lifexp_m+lifexp_sd), width=.2, position=position_dodge(0.05))
  

plot2 = ggplot(data_continent_2002, aes(x=gdp_m, y=lifexp_m, col=continent)) +
  geom_point(size=4) +
  ylim(35, 85) +
  ylab("Espérance de vie (en 2002)") +
  xlim(1000, 28000) + 
...
On peut s'intéresser au coefficient de corrélation de ces deux paramètres afin d'investiguer davantage leur relation. On décide d'utiliser le coefficient de corrélation de spearman puisque la relation ne semble pas linéaire.
