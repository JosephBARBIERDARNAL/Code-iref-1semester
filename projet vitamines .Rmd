...
On peut aussi calculer la contribution des variables au premier axe :
...
results_with_suppind = PCA(data_with_outliers, graph = FALSE, quanti.sup = c(10:13), quali.sup=14)

#show observations projection
fviz_pca_ind(results_with_suppind, axes = c(1,2), habillage = 14, palette = c("black", "coral3"))
```
...
Afin de séparer chaque variable en 3 modalités (faible, modéré et élevé), on décide d'utiliser comme breaks les quartiles. Cependant, étant donné que ces derniers imposent des effectifs de même taille dans chaque catégorie, il est possible d'avoir des 1ers et 3èmes quartiles égaux. Afin de corriger ce problème, on décide d'ajouter un très léger bruit aux variables initiales, assurant ainsi qu'elles soient toutes différentes.
...
to_modalities = function(var){
  ...
  #add small noise in order to avoid exact same values for quantiles
